            ...
            <AnimatePresence>
                {showSuccess && (
                    <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#F1F3F7]/20 dark:bg-[#2D3548]/20 backdrop-blur-3xl"
                    >
                        <motion.div
                            initial={{ scale: 0.5, opacity: 0 }}
                            animate={{ scale: 1, opacity: 1 }}
                            exit={{ scale: 0.5, opacity: 0 }}
                            className="bg-[#E5E7EB] dark:bg-[#1A1F2E] p-12 rounded-[3.5rem] shadow-2xl border border-[#E2E5E9] dark:border-[#3D4556] text-center relative overflow-hidden"
                        >
                            <motion.div
                            ...
                            >
                                ...
                                <h2 className="text-3xl font-black text-[#0F1419] dark:text-[#E8EAED] mb-2">Success!</h2>
                                <p className="text-[#64748B] dark:text-[#868D9D] font-medium">You have been successfully enrolled.</p>
                            </motion.div>
                ...
                {dialogOpen && selectedCourse && (
                    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
                        ...
                        <motion.div
                            ...
                            className="bg-[#E5E7EB] dark:bg-[#1A1F2E] w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden relative z-10 border border-[#E2E5E9]/50 dark:border-[#3D4556]/50 max-h-[85vh] flex flex-col"
                        >
                            <div className="p-8 border-b border-[#E2E5E9] dark:border-[#3D4556] flex justify-between items-center">
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">{selectedCourse.code}</span>
                                    </div>
                                    <h2 className="text-2xl font-black text-[#0F1419] dark:text-[#E8EAED] leading-tight">{selectedCourse.name}</h2>
                                </div>
                                <button
                                    onClick={() => { setDialogOpen(false); setSelectedCourse(null); }}
                            ...
                            <div className="p-8 overflow-y-auto custom-scrollbar space-y-8">
                                <div>
                                    ...
                                    <p className="text-sm text-[#64748B] dark:text-[#868D9D] leading-relaxed">
                                    ...
                                    <div className="space-y-3">
                                        {selectedCourse.materials && selectedCourse.materials.length > 0 ? (
                                            selectedCourse.materials.map((material) => (
                                                <div key={material._id} className="p-4 rounded-2xl bg-[#F1F3F7] dark:bg-[#2D3548] border border-[#E2E5E9] dark:border-[#3D4556] flex items-center justify-between group">
                                                    <div className="flex items-center gap-3">
                                                        <div className="p-2.5 rounded-xl bg-[#E5E7EB] dark:bg-[#1A1F2E] text-indigo-500 shadow-sm border border-[#E2E5E9] dark:border-[#3D4556]">
                                                            <FileText size={18} />
                                                        </div>
                                                        <div>
                                                            <p className="font-bold text-[#0F1419] dark:text-[#E8EAED] text-sm">{material.title}</p>
                                            ...
                                            <div className="py-12 border-2 border-dashed border-[#E2E5E9] dark:border-[#3D4556] rounded-3xl text-center">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="p-4 rounded-2xl bg-blue-50/50 dark:bg-blue-900/20 border border-blue-500/20">
                                        <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-1">Schedule</p>
                                        <p className="text-sm font-bold text-[#475569] dark:text-[#B8BDC6]">{selectedCourse.schedule || "TBA"}</p>
                                    </div>
                                    <div className="p-4 rounded-2xl bg-purple-50/50 dark:bg-purple-900/20 border border-purple-100 dark:border-purple-800">
                                        ...
                                        <p className="text-sm font-bold text-[#475569] dark:text-[#B8BDC6]">{selectedCourse.room || "TBA"}</p>
                                    </div>
                                </div>
                            </div>
                        </motion.div>
...
const CourseCard = ({ course, type, onEnroll, enrollingId }) => {
    ...
    return (
        <motion.div
            ...
            whileHover={{ y: -8 }}
            className="group relative bg-[#E5E7EB]/90 dark:bg-[#1A1F2E]/90 rounded-[2.5rem] p-6 shadow-xl shadow-indigo-100/50 dark:shadow-none border border-indigo-50/50 dark:border-slate-800 overflow-hidden transition-all duration-500 hover:bg-white"
        >
            {/* Visual Accents */}
            ...
            <div className="flex items-start justify-between mb-6">
                <div className="flex flex-col gap-1">
                    <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">{course.code}</span>
                    <h3 className="text-xl font-black text-[#0F1419] dark:text-[#E8EAED] leading-tight group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                        {course.name}
                    </h3>
                </div>
                {type === 'available' && (
            ...
            <div className="space-y-4 mb-8">
                <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-[#E5E7EB] dark:bg-[#242B3D] flex items-center justify-center overflow-hidden border border-[#E2E5E9] dark:border-[#3D4556]">
                        <img
                    ...
                    <span className="text-sm font-bold text-[#64748B] dark:text-[#868D9D]">
                ...
                <div className="grid grid-cols-2 gap-2">
                    <div className="p-3 rounded-2xl bg-indigo-50/30 dark:bg-slate-800/50 border border-indigo-100/20 dark:border-slate-800">
                        <p className="text-[10px] font-black text-[#64748B] dark:text-[#868D9D] dark:text-[#64748B] dark:text-[#868D9D] uppercase tracking-tighter mb-1">Credits</p>
                    ...
                    <div className="p-3 rounded-2xl bg-indigo-50/30 dark:bg-slate-800/50 border border-indigo-100/20 dark:border-slate-800">
                        <p className="text-[10px] font-black text-[#64748B] dark:text-[#868D9D] dark:text-[#64748B] dark:text-[#868D9D] uppercase tracking-tighter mb-1">Enrolled</p>
                        <p className="text-xs font-bold text-indigo-600 dark:text-white flex items-center gap-1.5">
            ...
            <div className="pt-6 border-t border-[#E2E5E9] dark:border-[#3D4556]">
                {type === 'available' && (
                    <button
                ...
                {type === 'completed' && (
                    <div className="w-full py-4 rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 text-emerald-400 border border-emerald-500/20 flex items-center justify-center gap-2 font-black text-xs uppercase tracking-[0.2em]">
                        <CheckCircle className="h-4 w-4" />
                        RECORDS STORED
                    </div>
                )}
...
const EmptyState = ({ icon: Icon, message }) => (
    <div className="text-center py-12 text-[#64748B] dark:text-[#868D9D]">
