            ...
            <div className="grid grid-cols-1 gap-3">
                <div className="flex items-center justify-between p-4 rounded-3xl bg-amber-500/5 border border-amber-500/10">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs font-black text-slate-800 dark:text-slate-100 uppercase tracking-tighter">Pending</span>
                    </div>
                ...
                <div className="flex items-center justify-between p-4 rounded-3xl bg-emerald-500/5 border border-emerald-500/10">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs font-black text-slate-800 dark:text-slate-100 uppercase tracking-tighter">Approved</span>
                    </div>
                    ...
                    <BentoCard title="Attendance Insights" icon={TrendingUp} className="md:col-span-2" delay={0.4}>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-2">
                            {attendanceOverview.slice(0, 4).map((item, idx) => {
                                ...
                                return (
                                    <div key={idx} className="p-6 rounded-[2.5rem] bg-slate-50 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800 transition-transform hover:translate-y-[-4px]">
                                        <div className="flex justify-between items-center mb-5">
                                            <div>
                                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Subject</p>
                                                <p className="text-sm font-black text-slate-800 dark:text-slate-100 truncate max-w-[160px]">{item.subject}</p>
                                            </div>
                                            <div className={cn(
                            ...
                            {grades.length > 0 ? grades.map((grade, idx) => (
                                <div key={idx} className="flex items-center justify-between p-4 rounded-3xl bg-slate-50 dark:bg-slate-800/30 border border-slate-100/50 dark:border-slate-800/50">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 rounded-2xl bg-indigo-500 text-white flex items-center justify-center font-black text-lg shadow-lg shadow-indigo-500/20">
                                            {grade.grade}
                                        </div>
                                        <div>
                                            <p className="text-xs font-black text-slate-800 dark:text-slate-100 line-clamp-1">{grade.subject}</p>
                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Verified Entry</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-xs font-black text-slate-800 dark:text-slate-100">{grade.marks}</p>
                                        <div className="w-8 h-1 bg-slate-200 dark:bg-slate-700 rounded-full mt-1 ml-auto overflow-hidden">
                                            <div className="h-full bg-indigo-500" style={{ width: `${(grade.marks / grade.maxMarks) * 100}%` }} />
                                        </div>
                                    </div>
                                </div>
                            )) : (
                                    ...
                                    <div>
                                        <div className="flex items-center justify-between mb-5">
                                            <span className={cn(
                                                ...
                                                notice.type === 'urgent' ? "bg-rose-500 text-white" : "bg-indigo-500 text-white"
                                            )}>{notice.type}</span>
                                            <div className="w-8 h-8 rounded-full bg-white dark:bg-slate-900 flex items-center justify-center opacity-0 group-hover/ann:opacity-100 transition-opacity">
                                                <ArrowRight size={14} className="text-indigo-500" />
                                            </div>
                                        </div>
                                        <h4 className="font-black text-slate-800 dark:text-white mb-3 leading-[1.2] line-clamp-2 text-base">{notice.title}</h4>
                                        <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-3 leading-relaxed">{notice.content}</p>
                                    </div>
                                    <div className="mt-8 flex items-center justify-between pt-5 border-t border-slate-200/50 dark:border-slate-700/50">
                                        <div className="flex items-center gap-2">
                                            <div className="w-7 h-7 rounded-full bg-indigo-500 flex items-center justify-center text-[10px] font-black text-white">
                                                {notice.author[0]}
                                            </div>
