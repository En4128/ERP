                ...
                {statCards.map((card, idx) => (
                    <GlassCard key={idx} delay={idx * 0.1} className="relative group p-6 overflow-hidden">
                        ...
                        <div className="relative z-10">
                            <div className={`w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-gradient-to-br ${card.gradient} flex items-center justify-center text-white mb-3 md:mb-4 shadow-lg shadow-gray-200 dark:shadow-none`}>
                                <card.icon size={20} md:size={24} />
                            </div>
                            <h3 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white mb-1 tracking-tight">{card.value}</h3>
                            <p className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-wider">{card.title}</p>
                        </div>
                    </GlassCard>
                ))}
            </div>
                ...
                {activeModal === 'permissions' && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md">
                        <motion.div
                            ...
                            exit={{ y: 20, opacity: 0 }}
                            className="bg-white dark:bg-slate-900 w-full max-w-4xl max-h-[90vh] md:h-[85vh] rounded-[2rem] md:rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 flex flex-col"
                        >
                            <div className="p-4 md:p-8 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 z-10">
                                <div className="flex justify-between items-start mb-4 md:mb-6">
                                    <div>
                                        <h3 className="text-xl md:text-3xl font-black text-slate-800 dark:text-white flex items-center gap-2 md:gap-3">
                                            <ShieldCheck className="text-teal-500" size={24} md:size={32} />
                                            User Management
                                        </h3>
                                        <p className="text-[10px] md:text-sm text-slate-500 font-bold uppercase tracking-widest mt-1 md:mt-2">Security & Access Control</p>
                                    </div>
                                    <button
                                        onClick={() => { setActiveModal(null); setUserSearch(''); }}
                                        className="p-3 md:p-4 bg-slate-100 hover:bg-rose-500 dark:bg-slate-800 dark:hover:bg-rose-500 text-slate-500 hover:text-white rounded-xl md:rounded-2xl transition-all"
                                    >
                                ...
                                <div className="relative">
                                    <Search className="absolute left-4 md:left-6 top-1/2 -translate-y-1/2 text-slate-400" size={16} md:size={20} />
                                    <input
                                        type="text"
                                        placeholder="Search by name, email, or role..."
                                        value={userSearch}
                                        onChange={(e) => setUserSearch(e.target.value)}
                                        className="w-full pl-12 md:pl-16 pr-4 md:pr-6 py-3 md:py-5 rounded-xl md:rounded-2xl bg-slate-50 dark:bg-slate-800/50 border-2 border-slate-100 dark:border-slate-700/50 focus:border-teal-500 transition-all outline-none font-bold text-slate-800 dark:text-white shadow-inner text-xs md:text-sm"
                                    />
                                </div>
                            </div>

                            <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-4 bg-slate-50/50 dark:bg-slate-950/50">
                                {userLoading ? (
                                    ...
                                    filteredUsers.map(user => (
                                        ...
                                        >
                                            <div className="flex items-center gap-5">
                                                <div className={`w-10 h-10 md:w-14 md:h-14 rounded-xl md:rounded-2xl flex items-center justify-center text-white font-black text-sm md:text-xl shadow-lg transform group-hover:scale-110 transition-transform flex-shrink-0 ${user.role === 'faculty' ? 'bg-gradient-to-br from-[#0066CC] to-blue-400 shadow-blue-200' : 'bg-gradient-to-br from-[#0066CC] to-cyan-500 shadow-blue-200'
                                                    }`}>
                                                    {user.name.charAt(0)}
                                                </div>
                                                <div className="min-w-0">
                                                    <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
                                                        <h4 className="font-black text-sm md:text-lg text-slate-800 dark:text-white leading-tight truncate">{user.name}</h4>
                                                        <span className={`w-fit px-2 py-0.5 md:px-2.5 md:py-1 rounded-lg text-[8px] md:text-[10px] font-black uppercase tracking-wider ${user.role === 'faculty' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30' : 'bg-blue-100 text-blue-700 dark:bg-blue-900/30'
                                                            }`}>
                                                            {user.role}
                                                        </span>
                                                    </div>
                                                    <p className="text-[10px] md:text-sm text-slate-500 font-bold mt-0.5 md:mt-1 truncate">{user.email}</p>
                                                </div>
                                            </div>

                                            <div className="flex flex-col sm:flex-row items-end sm:items-center gap-2 md:gap-4 ml-2">
                                                {user.isBlocked && (
                                                    <span className="flex items-center gap-1 px-2 py-1 md:px-4 md:py-2 rounded-lg md:rounded-xl bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 text-[8px] md:text-[10px] font-black uppercase tracking-widest animate-pulse border border-rose-100 dark:border-rose-900/50">
                                                        <Ban size={10} md:size={12} />
                                                        Blocked
                                                    </span>
                                                )}
                                                <button
                                                    onClick={() => handleToggleBlock(user._id)}
                                                    className={`px-4 py-2 md:px-6 md:py-3 rounded-xl md:rounded-2xl text-[8px] md:text-[10px] font-black uppercase tracking-widest transition-all ${user.isBlocked
